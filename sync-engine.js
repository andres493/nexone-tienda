...
function findDuplicate(products, supplierId, name) {
  return products.find(p => {
    if (supplierId && p.supplierId && p.supplierId === supplierId && p.provider) return true;
...
async function runSyncJob(jobId, broadcast) {
  ...
  if (!provider) {
    ...
    addLog(db, jobId, 'error', 'Provider not found: ' + job.provider);
    writeDB(db);
  ...
  if (!provider.isConfigured) {
    ...
    addLog(db, jobId, 'error', provider.name + ' API not configured');
    writeDB(db);
    if (broadcast) broadcast('sync-job-updated', job);
    return;
  }
  ...
  addLog(db, jobId, 'info', `Starting sync: ${job.provider} | ${job.searchType}: ${job.searchQuery}`);
  writeDB(db);
  ...
  try {
    while (hasMore && totalImported + totalUpdated + totalSkipped < maxProducts) {
      ...
      if (!activeJob || activeJob.cancelled) {
        ...
        addLog(db, jobId, 'warn', 'Sync cancelled by user');
        writeDB(db);
        if (broadcast) broadcast('sync-job-updated', db.syncJobs.find(j => j.id === jobId));
        return;
      }
      if (activeJob && activeJob.paused) {
        ...
        writeDB(db);
        if (broadcast) broadcast('sync-job-updated', db.syncJobs.find(j => j.id === jobId));
        while (activeJob && activeJob.paused && !activeJob.cancelled) {
          await delay(1000);
        }
        if (activeJob && activeJob.cancelled) {
          updateJob(db, jobId, { status: 'cancelled' });
      ...
      try {
        ...
        totalErrors++;
        addLog(db, jobId, 'error', `Page ${page} API error: ${e.message}`);
        writeDB(db);
        hasMore = false;
        break;
      }
      ...
      if (result.error) {
        totalErrors++;
        addLog(db, jobId, 'error', `Page ${page}: ${result.error}`);
        hasMore = false;
        break;
      }
      ...
      for (const raw of products) {
        ...
        try {
          ...
          const existing = findDuplicate(allProducts, normalized.supplierId, normalized.name);
          if (existing) {
            const needUpdate = existing.price !== normalized.price || existing.image !== normalized.image;
            if (needUpdate) {
          ...
          totalErrors++;
          addLog(db, jobId, 'warn', `Product skipped: ${e.message}`);
        }
      }

      writeDB({ ...dbFresh, products: allProducts });
      updateJob(db, jobId, { progress: { current: page, total: Math.ceil((result.total || 0) / 50) }, stats: { imported: totalImported, updated: totalUpdated, skipped: totalSkipped, errors: totalErrors } });
      if (broadcast) broadcast('sync-job-progress', { jobId, progress: { current: page, total: Math.ceil((result.total || 0) / 50) }, stats: { imported: totalImported, updated: totalUpdated, skipped: totalSkipped, errors: totalErrors } });
    ...
    const finalStatus = totalImported + totalUpdated + totalSkipped >= maxProducts ? 'completed' : 'completed';
    updateJob(db, jobId, {
      status: finalStatus,
      completedAt: new Date().toISOString(),
      stats: { imported: totalImported, updated: totalUpdated, skipped: totalSkipped, errors: totalErrors },
    });
    addLog(db, jobId, 'info', `Sync completed: ${totalImported} imported, ${totalUpdated} updated, ${totalSkipped} skipped, ${totalErrors} errors`);
    writeDB(db);
    if (broadcast) broadcast('sync-job-updated', db.syncJobs.find(j => j.id === jobId));
    ...
    addLog(db, jobId, 'error', `Sync failed: ${e.message}`);
    writeDB(db);
    if (broadcast) broadcast('sync-job-updated', db.syncJobs.find(j => j.id === jobId));
  } finally {
...
function createJob(params, broadcast) {
  ...
  addLog(db, job.id, 'info', `Job created: ${job.provider} | ${job.searchType}: ${job.searchQuery}`);
  writeDB(db);
...
function pauseJob(jobId) {
  ...
  const db = ensureConfig(readDB());
  updateJob(db, jobId, { status: 'paused' });
  addLog(db, jobId, 'info', 'Sync paused by user');
  writeDB(db);
...
function resumeJob(jobId, broadcast) {
  ...
  if (active) {
    ...
    const db = ensureConfig(readDB());
    updateJob(db, jobId, { status: 'running' });
    addLog(db, jobId, 'info', 'Sync resumed');
    writeDB(db);
    return db.syncJobs.find(j => j.id === jobId);
  }
  const db = ensureConfig(readDB());
...
function cancelJob(jobId) {
  ...
  const db = ensureConfig(readDB());
  updateJob(db, jobId, { status: 'cancelled' });
  addLog(db, jobId, 'info', 'Sync cancelled');
  writeDB(db);
  return db.syncJobs.find(j => j.id === jobId);
}
...
function updateConfig(updates, broadcast) {
  ...
  if (updates.providers) {
    db.syncConfig.providers = { ...db.syncConfig.providers, ...updates.providers };
  }
  writeDB(db);
